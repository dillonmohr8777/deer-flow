...
describe("workspace invitations", () => {
  it("keeps a fragment invite through StrictMode, hides its URL token and never auto-accepts", async () => {
    ...
    const fetcher = rs.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ email: "person@example.com", workspace_name: "Team", expires_at: "2026-10-01T00:00:00Z", requires_login: false })),
    );
    render(<StrictMode><InvitePage /></StrictMode>);
    await screen.findByLabelText("New password");
    expect(window.location.hash).toBe("");
    expect(fetcher.mock.calls.every(([path]) => path === "/api/v1/auth/invitations/inspect")).toBe(true);
    expect(screen.queryByText(/Missing invite token/)).toBeNull();
    fireEvent.change(screen.getByLabelText("New password"), { target: { value: "one-strong-password" } });
  ...
  it("accepts an existing account password without imposing the new-account length rule", async () => {
    ...
    const fetcher = rs.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      new Response(JSON.stringify({ email: "person@example.com", workspace_name: "Team", expires_at: "2026-10-01T00:00:00Z", requires_login: true })),
    ).mockResolvedValueOnce(new Response(JSON.stringify({ detail: "Expired invitation" }), { status: 403 }));
    render(<InvitePage />);
