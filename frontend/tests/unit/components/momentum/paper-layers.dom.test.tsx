...
const appearance = rs.hoisted(() => ({
  value: {
    preferences: { treatment: "paper" as const, motion: true, logo: null, label: "" },
    reducedMotion: false,
    visible: true,
  },
}));
...
function motion(allowed: { motion?: boolean; reducedMotion?: boolean }) {
  appearance.value = {
    ...appearance.value,
    preferences: { ...appearance.value.preferences, motion: allowed.motion ?? true },
    reducedMotion: allowed.reducedMotion ?? false,
  };
}

const LAYERS = ["/momentum/brain/kraft.webp", "/momentum/brain/body.webp", "/momentum/brain/folds.webp"];
...
describe("PaperLayers", () => {
  it("renders only the flattened image below 40px, no stage", () => {
    const { container } = render(
      <PaperLayers layers={LAYERS} flatSrc={FLAT} size={32} alt="Dillon Brain" />,
    );
  ...
  it("holds fully static under reduced motion or the app motion switch off, even at hero size", () => {
    for (const setting of [{ reducedMotion: true }, { motion: false }] as const) {
      motion(setting);
      const { container, unmount } = render(
        <PaperLayers layers={LAYERS} flatSrc={FLAT} size={160} state="working" alt="Dillon Brain" />,
      );
      const imgs = container.querySelectorAll("img");
      expect(imgs).toHaveLength(1);
      expect(imgs[0]?.getAttribute("src")).toBe(FLAT);
  ...
  it("applies the breathe class only while state is working", () => {
    const { container: idle } = render(
      <PaperLayers layers={LAYERS} flatSrc={FLAT} size={160} state="idle" alt="Dillon Brain" />,
    );
  ...
  it("gives alt only to the top layer; the rest stay aria-hidden", () => {
    const { container } = render(
      <PaperLayers layers={LAYERS} flatSrc={FLAT} size={160} alt="Dillon Brain" />,
    );
