 ...
 */
export const COMIC_BOOT_SCRIPT = `(function(){try{var d=document.documentElement,s=location.search,p=false,A=${JSON.stringify(COMIC_ATTR)};try{p=sessionStorage.getItem(${JSON.stringify(COMIC_SESSION_KEY)})==="1"}catch(e){}var r=!!(window.matchMedia&&matchMedia("(prefers-reduced-motion: reduce)").matches);if(!r&&!/[?&]look=current\\b/.test(s)&&(!p||/[?&]replay\\b/.test(s))){d.setAttribute(A,"play");if("scrollRestoration" in history)history.scrollRestoration="manual";setTimeout(function(){if(d.getAttribute(A)==="play"&&!d.hasAttribute(${JSON.stringify(COMIC_RUNNING_ATTR)})){d.setAttribute(A,"done");if("scrollRestoration" in history)history.scrollRestoration="auto"}},Math.max(0,${LOAD_DEADLINE_MS}-(window.performance?performance.now():0)))}}catch(e){}})();`;
