...
Composer drafts are tab-scoped browser state. `core/threads/composer-draft.ts` stores only text plus the selected slash-skill name in `sessionStorage`, keyed by user, agent, and logical conversation scope. New-chat pages pass the stable scope `"new"` because their runtime `threadId` is a fresh UUID on every reload; established conversations use their real thread ID. `InputBox` waits for enabled skills before restoring a skill chip, degrades a missing/disabled skill back to editable slash text, and clears the stored draft through `SendMessageOptions.onSent` only after the send passes the in-flight guard. Attachments, sidecar quotes, voice state, and polish undo state are not persisted. Any other trigger that wants a fresh chat to open with a skill pre-activated (e.g. `welcome-clients-card.tsx`'s "Set up my clients") writes that same `"new"`-scoped draft with `writeComposerDraft`/`buildComposerDraftKey` before navigating, rather than inventing a URL param or a bespoke first-message channel — `InputBox`'s existing hydration renders it.

`welcome-clients-card.tsx` shows once after the experience-mode chooser resolves (`settings.context.experience_mode` non-null). Its dismissal preference (`welcome_clients_dismissed`) round-trips through `/api/v1/auth/preferences` with its own direct GET/PATCH, the same pattern `experience-mode-chooser.tsx` uses for `experience_mode` — deliberately outside the passive four-field `activatePreferences`/`preferencesFromSettings` sync below, because a "have I already decided this" gate needs a synchronous fetched answer on mount, not a value that may still be the pre-sync default.
