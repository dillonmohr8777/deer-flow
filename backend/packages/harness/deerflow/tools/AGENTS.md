...
3. **Built-in tools**:
   ...
   - `onboard_client_workspace` (`tools/builtins/client_onboarding_tool.py`) - One atomic action for the Momentum welcome skill's `batch_task` items: create-or-update a `client_id`-linked project, write a shelf document, store memory facts (best-effort per fact), and create a paused weekly-cron scheduled task. Always bound (like `ask_clarification`), so a `general-purpose` subagent inherits it too. Since tool execution never sets the ambient `_storage_context` the way a request does, it resolves the caller's active organization itself via `private_organization_for_user` (same resolver `ProjectRepository.create`/`ScheduledTaskRepository.create` use) before touching `ClientRepository`, which is organization-scoped with no `user_id` column.
