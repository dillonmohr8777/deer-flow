...
- `migrations/versions/0035_user_mfa.py` — creates `user_mfa` (TOTP secret encrypted at rest, `enabled_at`, hashed recovery codes as JSON), FK-cascaded on `users.id` delete; chains after `0034_clients`. New table, so the bootstrap forward-compat floor is unchanged.
