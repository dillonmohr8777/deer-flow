...
@pytest.mark.anyio
async def test_experience_mode_round_trips_and_rejects_invalid_value(api):
    client, _ = api
    path = "/api/v1/auth/preferences"
    assert (await client.get(path)).json()["experience_mode"] is None
    assert (await client.patch(path, json={"experience_mode": "easy"})).status_code == 204
    assert (await client.get(path)).json()["experience_mode"] == "easy"
