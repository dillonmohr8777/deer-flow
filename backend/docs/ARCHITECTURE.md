...
**Evidence check (verify_quote pilot).** `verify_quote(document_id, quote)` (same registration gate, §7.3) checks whether an exact quote, 20 to 600 characters, appears in one shelf document's text after folding whitespace runs to a single space and curly/smart quote characters to straight ones, then reports the matched character offsets and about 200 characters of surrounding context in that normalized text, or a plain not-found result. A foreign, cross-organization, or trashed document reports the same "no longer on the shelf" error as a missing one, never a distinguishable access-denied response. A found quote confirms the text exists in the document; it does not confirm the quote supports whatever claim cites it, so it is a necessary check, not a sufficient one. The lead agent prompt gains a short citation instruction (cite as `[doc:<id>]`, call `verify_quote` before relying on a quote, say plainly when a claim cannot be confirmed), appended to the transient project message right after the `<documents>` block and rendered only when that block itself renders (a nonempty shelf).
