...
Postgres afterward: `0032_org_delegation_backfill` -- matches this checkout's
...
1. **Fresh backup.** Snapshot `deer-flow_gateway-data` into a new volume with
   `backup_volume.py` (see "Back up" above). Keep it -- it is the rollback
...
6. **Verify:** `python backend/scripts/verify_cutover.py <extracted
   deerflow.db> <postgres DSN>` -- must print `PASS` before continuing.
